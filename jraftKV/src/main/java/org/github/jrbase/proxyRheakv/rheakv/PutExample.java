...
public class PutExample {
    ...
    public static void put(final RheaKVStore backendProxy) {
        ...
        final CompletableFuture<Boolean> r1 = backendProxy.put("1", value);
        if (FutureHelper.get(r1)) {
            LOG.info("Async put 1 {} success.", readUtf8(backendProxy.bGet("1")));
        }

        final CompletableFuture<Boolean> r2 = backendProxy.put(writeUtf8("2"), value);
        if (FutureHelper.get(r2)) {
            LOG.info("Async put 2 {} success.", readUtf8(backendProxy.bGet("2")));
        }

        final boolean r3 = backendProxy.bPut("3", value);
        if (r3) {
            LOG.info("Sync put 3 {} success.", readUtf8(backendProxy.bGet("3")));
        }

        final boolean r4 = backendProxy.bPut(writeUtf8("4"), value);
        if (r4) {
            LOG.info("Sync put 4 {} success.", readUtf8(backendProxy.bGet("4")));
        }
        ...
        final CompletableFuture<Boolean> r5 = backendProxy.put(entries);
        ...
        final boolean r6 = backendProxy.bPut(entries);
        if (r6) {
            for (final KVEntry entry : entries) {
